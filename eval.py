...
##############
from models.down_stream_model import Classifier
...
def main(args):
    ...
    dataset_val = ImageNetDataset(
        data_root=args.imagenet_path, image_size=args.image_size, n_class=args.class_num, partition="val", device=device
    )
    ...
    ##Load LLaMA
    llama = LlamaModel.from_pretrained("/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/Meta-Llama-3-8B")
    llama.eval()
    llama.to(args.gpu)
    classifier = Classifier(args)
    checkpoint_path = "/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/classifier/classifier-last.pth" 
    checkpoint = torch.load(checkpoint_path, map_location='cpu')

    classifier.load_state_dict(checkpoint['model'])
    classifier.eval()
    classifier.to(args.gpu)
    # tokenizer = AutoTokenizer.from_pretrained("/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/Meta-Llama-3-8B")
    ...
    for data_iter_step, [images, clip_image, label_cls] in enumerate(
        ...
        with torch.no_grad():
            visual_embeddings, visual_codebook_indices = model(x, data_iter_step, step=0, is_val=True)
        with torch.no_grad():
            outputs = llama(visual_codebook_indices)
        logits = outputs.last_hidden_state # torch.Size([bs, 256, 4096])
        prediction = classifier(logits[:, -1, :])
        criterion = torch.nn.CrossEntropyLoss()
        loss = criterion(prediction, label_cls)
        acc1, acc5 = accuracy(prediction, label_cls, topk=(1, 5))     
        print(acc1)
        #### visual_embeddings = torch.Size([16, 768, 32, 32])
        # ###Load LLaMA
        ...
        #     max_batch_size=4,
        # )
        # llama_embeddings = generator.model.tok_embeddings.weight.detach().cpu().numpy() #  (32000, 4096)

        ## in case need to add text later
        # text_tokenizer = Tokenizer(model_path=args.llama_model_path + "/tokenizer.model")
        # text_embedding = F.embedding(visual_codebook_indices, generator.model.tok_embeddings.weight)
        # prompt_tokens = visual_codebook_indices.tolist()
        # generation_tokens, hidden_states,_ = generator.generate(
        ...
        # prompt = [{"prompt": text_tokenizer.decode(t)} for t in prompt_tokens]
        # print(prompt)
        # results = [{"generation": text_tokenizer.decode(t)} for t in generation_tokens]
