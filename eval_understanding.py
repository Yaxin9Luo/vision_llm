...
class MiniImageNetDataset(Dataset):
    def __init__(self, data_root, image_size, time, model_path, vision_tokenizer, text_token_embedding, partition="val",way=2, shot=1, device="cpu", induction=1):
        ...
        with open("imagenet_split/" + partition + "/class_labels.txt") as f:
            for line in f.readlines():
                image_id, class_label = line.strip('\n').split(",")
                if int(class_label) < 1000: #only select 100 class
                    ##
                    if partition == "train":
                        self.image_ids.append(image_id)
                    elif partition == "val":
                        ...
                        class_name = imagenet_dict[str(np.int64(class_label))][0]
                        self.image_ids.append(os.path.join("val", class_name, image_name))
                    self.class_names.append(int(class_label))
    ...
    def __getitem__(self, index):
        ...
        with torch.no_grad():
            visual_embeddings, visual_codebook_indices = self.vision_tokenizer(images, 0, step=0, is_val=True, k=args.token_num)
        ...
        for j in range(0, args.repeat+1):
            for i in range(0, len(select_texts)):
                ...
                vis_feature = self.text_token_embedding(token_idx)
        ...
        #     vis_texts = vis_texts + text
        vis_token = torch.tensor(self.text_tokenizer.encode(token_idx, bos=False, eos=False), dtype=torch.int64).to(self.device).unsqueeze(0)
...
def get_args_parser():
    ...
    parser.add_argument("--imagenet_path", default="/data/all", type=str, help="path of llama model")
...
def main(args):
    ...
    if os.path.exists(os.path.join(args.output_dir, "%d-k_%d-way_%d-shot_%d-repeat_%d-induction.npy"%(args.token_num, args.way, args.shot, args.repeat, args.induction))):
        ...
        dataset = MiniImageNetDataset(
            data_root=args.imagenet_path, image_size=args.image_size, time=time, model_path=args.llama_model_path, vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, way=args.way, shot=args.shot, device=device, induction=args.induction
        )
    else:
        dataset = MiniImageNetDataset(
            data_root=args.imagenet_path, image_size=args.image_size, time=0, model_path=args.llama_model_path, vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, way=args.way, shot=args.shot, device=device, induction=args.induction
        )
