...
def train_one_epoch(args, model: torch.nn.Module, vqgan:torch.nn.Module, data_iter_step:int, criterion: DistillationLoss,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            if args.use_patch_aug:
                ...
                with torch.no_grad():
                    visual_embeddings, visual_codebook_indices = vqgan(samples, data_iter_step, step=0, is_val=True)
                outputs = model(visual_codebook_indices)
                loss = criterion(samples, outputs, targets)
