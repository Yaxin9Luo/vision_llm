...
vqgan_config = {
    ...
    "output_dir": "/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/",
    "log_dir": "/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/",
    "disc_start": 10000,
    "use_cblinear": 0,
    "embed_dim": 768
}
...
def get_args_parser():
    ...
    # LLama layers
    parser.add_argument('--llama_path', type=str, default='/fsx/zhiqiang.shen/yaxinluo_codes/model_checkpoints/Meta-Llama-3-8B')
...
def main(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        print('I am rank {} using GPU {} on host {}'.format(args.rank,device_id, socket.gethostname()))
        ...
        if args.rank == 0 and args.wandb:
            wandb.init(project='ViTs', entity='yaxinluo999', name=f'{args.model}:{args.exp_name}', 
    ...
    config = load_config('/home/zhiqiang.shen/projects/visual_tokenizer/V2L-Tokenizer/vqgan_configs/v2l.yaml', display=True)
    vqgan = VQModel_LLaMA(args=vqgan_config, **config.model.params)
    for param in vqgan.parameters():
        param.requires_grad = False
    for data_iter_step, epoch in enumerate(range(args.start_epoch, args.epochs), start=0):

        if args.distributed:
            data_loader_train.sampler.set_epoch(epoch)

        train_stats = train_one_epoch(
            args, model, vqgan,criterion, data_loader_train,
