...
class ViTLLaMA(nn.Module):
    ...
    def __init__(
        ...
        print("****Using LLaMA2 Token Embedding****")
        checkpoint = torch.load("/home/zhiqiang.shen/projects/visual_tokenizer/LM4VisualEncoding/Vision_LLM/llama2_codebook_embeddings.pth", map_location="cpu")
        n_vision_words = checkpoint.shape[0]
        llama_embed_dim = checkpoint.shape[1]
        print("Word Number:%d" %(n_vision_words))
        print("Text Feature Dim:%d" %(llama_embed_dim))
        print("Visual Feature Dimension: ",self.num_features)
        print(f"Embedding Dimension is: {embed_dim}")
        self.tok_embeddings = Embedding(n_vision_words, llama_embed_dim)
    ...
    def forward_features(self, visual_codebook_indices):
        # x = self._pos_embed(x)
        # x = self.blocks(x)

        # x = self.llama_dim_mapper1(x)
        text_embedding = F.embedding(visual_codebook_indices, self.tok_embeddings.weight)
        x = self.llama(text_embedding)
