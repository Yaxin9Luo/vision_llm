...
import transformers
from transformers import LlamaConfig,AutoTokenizer,LlamaModel,GemmaModel
from llama_inference.llama import Tokenizer,Transformer
from models.models_v2l import VQModel_LLaMA 
from training.engine_training import train_one_epoch,train_one_epoch_stage2,save_llama_outputs,evaluate
import util.misc as misc
import socket
from models.down_stream_model import Classifier,codebook_projector
...
def get_args_parser():
    ...
    parser.add_argument("--llama_model_path", type=str, default='./llama',  help="llama weights path")
    # Model parameters
    parser.add_argument("--max_seq_len", type=int, default=512, metavar="LENGTH", help="the maximum sequence length")
    parser.add_argument("--vqgan_ckt", type=str, default="stage_1_llama_tuned-40.pth", help="Decoding Loss")
...
class PrecomputedFeatureDataset(Dataset):
    def __init__(self, feature_dir):
        self.feature_files = sorted([f for f in os.listdir(feature_dir) if f.startswith('feature_')])
        self.label_files = sorted([f for f in os.listdir(feature_dir) if f.startswith('label_')])
        self.image_id_files = sorted([f for f in os.listdir(feature_dir) if f.startswith('image_id_')])
        self.feature_dir = feature_dir

        assert len(self.feature_files) == len(self.label_files) == len(self.image_id_files), "Mismatch in number of files"

    def __len__(self):
        return len(self.feature_files)

    def __getitem__(self, idx):
        feature = np.load(os.path.join(self.feature_dir, self.feature_files[idx]))
        label = np.load(os.path.join(self.feature_dir, self.label_files[idx]))
        
        with open(os.path.join(self.feature_dir, self.image_id_files[idx]), 'r') as f:
            image_id = f.read().strip()

        return torch.from_numpy(feature).float(), torch.from_numpy(label).long(), image_id
    ...
    def __init__(self, data_root, image_size, max_words=30, n_class=1000, partition="train", device="cpu"):
        ...
        with open("imagenet_split/" + partition + "/class_labels.txt") as f:
            for line in f.readlines():
                ...
                if int(class_label) < n_class:
                    ##
                    if partition == "train":
                        # data augmentation
                        self.transform = albumentations.Compose([
                            albumentations.SmallestMaxSize(max_size=self.image_size),
                            albumentations.RandomCrop(height=self.image_size, width=self.image_size),
                            albumentations.HorizontalFlip(),
                            albumentations.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.1)
                        ])
                        ...
                        ######### data augmentation #########
                        self.transform = albumentations.Compose([
                            albumentations.SmallestMaxSize(max_size=128),
        ...
        # # If partition is "train", sample only 1% of the data
        # if partition == "train":
        ...
        #     self.class_labels = [self.class_labels[i] for i in sampled_indices]
        # if partition == "val":
        #     total_samples = len(self.image_ids)
...
def calculate_trainable_parameters(target_model):
    model_parameters = filter(lambda p: p.requires_grad, target_model.parameters())
    params = sum([np.prod(p.size()) for p in model_parameters])
    return "{:,}".format(params)

def main(args):
    # model_repoid_or_path='microsoft/Phi-3-mini-4k-instruct'
    ...
    # train_feature_dir = "/l/users/zhiqiang.shen/visual_tokenizer/data/llama_features/train_full_pretrained_llama"
    # dataset_train = PrecomputedFeatureDataset(train_feature_dir)
    # val_feature_dir = "/l/users/zhiqiang.shen/visual_tokenizer/data/llama_features/val"
    # dataset_val = PrecomputedFeatureDataset(val_feature_dir)
    ...
    vqgan = VQModel_LLaMA(args=args, **config.model.params)
    if "last" in args.vqgan_ckt:
        sd = torch.load(os.path.join(args.vqgan_ckt), map_location="cpu")["model"]
    else:
        sd = torch.load(os.path.join(args.vqgan_ckt), map_location="cpu")["state_dict"]
    ...
    #     num_labels=1000
    # )
    # llama = LlamaModel(config) # Random init Llama weights

    # llama = LlamaModel.from_pretrained("/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/Llama-2-7b-hf") # Load pretrained LLaMA weights
    llama = GemmaModel.from_pretrained("/l/users/zhiqiang.shen/visual_tokenizer/checkpoints/gemma-2b")
    llama.eval()
    llama.to(device_id)
    for param in llama.parameters():
        param.requires_grad = False
    classifier = Classifier(args)
    classifier.to(device_id)
    # projector = codebook_projector(args)
    # projector.to(device_id)
    print(f"Trainable parameters in VQGAN: {calculate_trainable_parameters(vqgan)}")
    print(f"Trainable parameters in LLaMA: {calculate_trainable_parameters(llama)}")
    print(f"Trainable parameters in Classifier: {calculate_trainable_parameters(classifier)}")
    # print(f"Trainable parameters in Projector: {calculate_trainable_parameters(projector)}")
    ...
    if args.distributed:
        classifier = torch.nn.parallel.DistributedDataParallel(classifier, device_ids=[device_id])
        # projector = torch.nn.parallel.DistributedDataParallel(projector, device_ids=[device_id])
        model_without_ddp = classifier.module
        # projector_without_ddp = projector.module
    else:
        model_without_ddp = classifier
        # projector_without_ddp = projector

    if args.train_classifier:
        params = list(model_without_ddp.parameters())
        # params = list(model_without_ddp.parameters()) + list(projector_without_ddp.parameters())
        optimizer = torch.optim.Adam(params, lr=args.lr, betas=(0.5, 0.9), eps=1e-7)

    loss_scaler = NativeScaler()

    # #auto resume
    # if os.path.exists(os.path.join(args.output_dir, 'classifier-last.pth')):
    #     ckpt = torch.load(os.path.join(args.output_dir, 'classifier-last.pth'), map_location="cpu")
    #     model_without_ddp.load_state_dict(ckpt["model"], strict=True)
    #     classifier_para.load_state_dict(ckpt["opt_ae"])
    #     loss_scaler.load_state_dict(ckpt["scaler_ae"])
    #     args = ckpt["args"]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        ######## save llama hidden states first #########
        # save_llama_outputs(vqgan, llama, data_loader_train, device, output_dir="/l/users/zhiqiang.shen/visual_tokenizer/data/llama_features/train_full_pretrained_llama")
        train_stats = train_one_epoch_stage2(
            llama, vqgan, classifier, data_loader_train, optimizer, device, epoch, loss_scaler, log_writer=log_writer, args=args
        )
        val_stats = evaluate(vqgan, llama, classifier, data_loader_val, device)
        
        misc.save_model_classifier_loss(
            args=args,
            model=classifier,
            model_without_ddp=model_without_ddp,
            # projector=projector,
            # projector_without_ddp=projector_without_ddp,
            optimizer=optimizer,
