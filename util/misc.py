...
def save_model_classifier_loss(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('classifier-last.pth')]
        for checkpoint_path in checkpoint_paths:
            opt_ae = optimizer
...
def save_model_classifier(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('classifier-%s.pth' % epoch_name)]
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'state_dict': model_without_ddp.state_dict(),
                # 'projector_state_dict': projector_without_ddp.state_dict(),
                'epoch': epoch,
                'args': args,
            }
...
def save_model_var(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('var_checkpoint-%s.pth' % epoch_name)]
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'state_dict': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
...
def save_model_last_var(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    output_dir = Path(args.output_dir)
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / ('var_checkpoint-last.pth')]
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
        ...
        client_state = {'epoch': epoch}
        model.save_checkpoint(save_dir=args.output_dir, tag="checkpoint-last", client_state=client_state)
